...
export class CTgBotService extends CMailableService implements OnModuleInit {
    ...
    public async onModuleInit(): Promise<void> {
        try {
            ...
            const data = await this.sendRequest("setWebhook", {url: `${cfg.backUrl}/api/admin/users/tg-event`, secret_token: tgbotWhToken});
    ...
    public async userNewtask(user: CUser, guide: CGuide, task: CTask): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-newtask", user.lang_id);
    ...
    public async userNewguide(user: CUser, guide: CGuide): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-newguide", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl; // will use in eval
            const content = eval("`"+mtd.content+"`");
            const statusCode = await this.sendMessage(user.tg_id, content);
            return statusCode;
    ...
    public async userGuideReminder(user: CUser, guide: CGuide): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-guidereminder", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl; // will use in eval
            const content = eval("`"+mtd.content+"`");
            const statusCode = await this.sendMessage(user.tg_id, content);
            return statusCode;
        } catch (err) {
            await this.errorsService.log("api.admin/CTgBotService.userGuideReminder", err);
            return -1;
        }
    }
    ...
    public async userNewarticle(user: CUser, article: CArticle): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-newarticle", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl; // will use in eval
            const content = eval("`"+mtd.content+"`");
            const statusCode = await this.sendMessage(user.tg_id, content);
            return statusCode;
        } catch (err) {
            await this.errorsService.log("api.admin/CTgBotService.userNewarticle", err);
            return -1;
        }
    }
    ...
    public async userDeadline(user: CUser, task: CTask): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-deadline", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl; // will use in eval
            const content = eval("`"+mtd.content+"`");
            const statusCode = await this.sendMessage(user.tg_id, content);
            return statusCode;
    ...
    public async userWelcome(user: CUser): Promise<number> {
        try {
            const mtd = await this.getMailtemplateData("user-tg-welcome", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl; // will use in eval
            const content = eval("`"+mtd.content+"`");
            const statusCode = await this.sendMessage(user.tg_id, content);
            return statusCode;
    ...
    protected async sendMessage(chat_id: number, content: string): Promise<number> {
        try {
    ...
    private async sendRequest(method: string, dto: IKeyValue<any> = null): Promise<any> {
        const tgbotToken = (await this.dataSource.getRepository(CSetting).findOneBy({p: "tgbot-token"}))?.v;
        const tgbotApiUrl = (await this.dataSource.getRepository(CSetting).findOneBy({p: "tgbot-apiurl"}))?.v;
    ...
    private async test(): Promise<void> {
        const user = await this.dataSource.getRepository(CUser).findOne({where: {id: 1879}, relations: ["lang"]});
        const guide = await this.dataSource.getRepository(CGuide).findOne({where: {id: 10171}, relations: ["translations"]});
        const task = await this.dataSource.getRepository(CTask).findOne({where: {id: 151164}, relations: ["translations"]});
