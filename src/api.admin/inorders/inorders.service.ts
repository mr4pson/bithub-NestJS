...
export class CInordersService {
  ...
  public async chunk(dto: IGetList): Promise<IResponse<CInorder[]>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CInordersService.chunk',
        err,
      );
  ...
  public async one(id: number): Promise<IResponse<CInorder>> {
    try {
      ...
      return data
        ? { statusCode: 200, data }
        : { statusCode: 404, error: 'inorder not found' };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.admin/CInordersService.one',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async delete(id: number): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CInordersService.delete',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CInordersService.deleteBulk',
        err,
      );
      return { statusCode: 500, error };
    }
