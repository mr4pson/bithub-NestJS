import { Controller, Post, Body, UseGuards, Req, UseInterceptors, UploadedFiles, Param } from "@nestjs/common";
...
export class CUsersController {
    ...
    public me(@Req() request: Request): Promise<IResponse<IUser>> {
        const visitor_id = this.jwtService.decode(request.headers["token"] as string)["id"] as number;
        return this.usersService.one(visitor_id, visitor_id);
    }

    @UseGuards(CUserGuard)
    @Post("one/:id")
    public one(@Param("id") id: string, @Req() request: Request): Promise<IResponse<IUser>> {
        const visitor_id = this.jwtService.decode(request.headers["token"] as string)["id"] as number;
    ...
    @Post("register")
    public register(@Body() dto: IUserRegister, @Req() request: Request): Promise<IResponse<IUserAuthData>> {
    ...
    @Post("enter-by-token")
    public enterByToken(@Body() dto: IUserEnterByToken, @Req() request: Request): Promise<IResponse<IUserAuthData>> {
        const tz = parseInt(request.headers["tz"]);
    ...
    @UseGuards(CUserGuard)
    @Post("children-chunk")
    public childrenChunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<IUser[]>> {
    ...
    @UseGuards(CUserGuard)
    @Post("referees-chunk")
    public refereesChunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<IUser[]>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
    ...
    @Post("update-me")
    public updateMe(@Body() fd: IJsonFormData, @UploadedFiles() uploads: Express.Multer.File[], @Req() request: Request): Promise<IResponse<IUser>> {
        const token = request.headers["token"] as string;
    ...
    public updatePassword(@Body() dto: IUserUpdatePassword, @Req() request: Request): Promise<IResponse<void>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"];
