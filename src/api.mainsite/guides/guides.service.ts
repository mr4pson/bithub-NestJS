...
export class CGuidesService {
  ...
  public async chunk(
    ...
    try {
      ...
      if (user_id) {
        ...
        if (user) {
          query = query
            ...
            .loadRelationCountAndMap(
              'tasks.viewings_count',
              'tasks.viewings',
              'viewings',
              (qb) => qb.where(`viewings.user_id='${user_id}'`),
            ) // отметки о просмотрах тасков пользователем
            .loadRelationCountAndMap(
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CGuidesService.chunk',
        err,
      );
  ...
  public async statChunk(
    ...
    try {
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CGuidesService.statChunk',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async statCompletions(
    ...
    try {
      const users = await this.dataSource
        ...
        .where(
          `(users.id='${user_id}' OR users.parent_id='${user_id}') AND users.active='1'`,
        )
      ...
      const completions = await this.dataSource
        ...
        .find({ where: { user_id: In(user_ids), task_id: In(task_ids) } });
      const data = this.buildCompletions(users, tasks, completions);
      return { statusCode: 200, data };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.mainsite/CGuidesService.statCompletions',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async favoritesChunk(
    ...
    try {
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CGuidesService.favoritesChunk',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async updateFavorition(
    ...
    try {
      if (dto.favorited) {
        const favorition = this.dataSource
          .getRepository(CFavorition)
          .create({ user_id, guide_id: dto.guide_id });
        await this.dataSource.getRepository(CFavorition).save(favorition);
      } else {
        await this.dataSource
          .getRepository(CFavorition)
          .delete({ user_id, guide_id: dto.guide_id });
      }

      return { statusCode: 200 };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.mainsite/CGuidesService.updateFavorition',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  private async buildChunkFilter(
    dtoFilter: any,
    user_id: number,
  ): Promise<string> {
    ...
    if (dtoFilter.created_at_less) {
      filter += ` AND guides.created_at <= '${this.appService.mysqlDate(
  ...
  private buildGuideMin(guide: CGuide, langs: CLang[], user: CUser): IGuide {
    ...
    if (user) {
      ...
      data.progress = guide.tasks?.length
        ? parseFloat(
            (
              (guide.tasks.filter(
                (t) => t.type === 'main' && t['completions_count'] === 1,
              ).length *
                100) /
              guide.tasks.filter((t) => t.type === 'main').length
            ).toFixed(1),
          )
        : 0;
      data.has_unviewed = !!guide.tasks?.filter(
        (t) =>
          t.created_at.getTime() > user.created_at.getTime() &&
          !t['viewings_count'],
      ).length;
    }
  ...
  private buildGuideFull(guide: CGuide, langs: CLang[]): IGuide {
    ...
    const data: IGuide = {
      ...
      price: guide.price,
      type: guide.type,
      steps_limit: guide.steps_limit,
      time: guide.time,
      status: guide.status,
      created_at: guide.created_at,
      favorited: guide['favoritions_count'] === 1,
      progress: guide.tasks?.length
        ? parseFloat(
            (
              (guide.tasks.filter(
                (t) => t.type === 'main' && t['completions_count'] === 1,
  ...
  private buildCompletions(
    ...
    for (const user of users) {
      const completionTasks: ICompletionTask[] = tasks.map((task) => ({
        task_id: task.id,
        task_type: task.type,
        completed: !!completions.find(
          (c) => c.user_id === user.id && c.task_id === task.id,
        ),
      }));
      ...
      const qMainTasks = completionTasks.filter(
        (t) => t.task_type === 'main',
      ).length;
      const qCompletedMainTasks = completionTasks.filter(
        (t) => t.task_type === 'main' && t.completed,
      ).length;
      const completionProgress = parseFloat(
        ((qCompletedMainTasks * 100) / qMainTasks).toFixed(1),
      );
      data.push({
        user: completionUser,
  ...
  private buildTask(task: CTask, langs: CLang[], now: Date): ITask {
    const data: ITask = {
      ...
      actual: task.actual_until
        ? task.actual_until.getTime() > now.getTime()
        : true,
  ...
  private async getUsersWithProgress(user_id): Promise<number[]> {
    const q = `
        ...
        `;
    const userIds = (await this.dataSource.query(q)).map(
      (x) => x.id,
    ) as number[];
    return userIds;
  }
