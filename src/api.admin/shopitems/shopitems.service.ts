...
export class CShopitemsService extends CImagableService {
    ...
    protected folder: string = "shopitems";
    ...
    public async chunk(dto: IGetList): Promise<IResponse<CShopitem[]>> {
        try {
            ...
            const predata = await this.dataSource.getRepository(CShopitem)
            ...
            const data = await this.dataSource.getRepository(CShopitem)
                .createQueryBuilder("shopitems")
                ...
                .orderBy({[sortBy]: sortDir})
                .getMany();
            const elementsQuantity = await this.dataSource.getRepository(CShopitem)
                .createQueryBuilder("shopitems")
            ...
            const error = await this.errorsService.log("api.admin/CShopitemsService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<CShopitem>> {
        try {
            const data = await this.dataSource.getRepository(CShopitem).findOne({where: {id}, relations: ["translations"]});
            return data ? {statusCode: 200, data} : {statusCode: 404, error: "shopitem not found"};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            const x = await this.dataSource.getRepository(CShopitem).findOneBy({id});
            await this.deleteUnbindedImgOnDelete([x], false);
            await this.dataSource.getRepository(CShopitem).delete(id);
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.delete", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {
            const xl = await this.dataSource.getRepository(CShopitem).findBy({id: In(ids)});
            await this.deleteUnbindedImgOnDelete(xl, false);
            await this.dataSource.getRepository(CShopitem).delete(ids);
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async archive(id: number): Promise<IResponse<void>> {
        try {
            await this.dataSource.getRepository(CShopitem).update({id}, {archived: true});
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.archive", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async archiveBulk(ids: number[]): Promise<IResponse<void>> {
        try {
            await this.dataSource.getRepository(CShopitem).update({id: In(ids)}, {archived: true});
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.archiveBulk", err);
            return {statusCode: 500, error};
        }
    }

    public async create(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CShopitem>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CShopitemsService.create", err);
            return {statusCode: 500, error};
        }
    }

    public async update(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CShopitem>> {
        try {
            const dto = JSON.parse(fd.data) as IShopitemUpdate
            const x = this.dataSource.getRepository(CShopitem).create(dto);
            const old = await this.dataSource.getRepository(CShopitem).findOneBy({id: x.id});
            await this.buildImg(x, uploads);
            await this.deleteUnbindedImgOnUpdate(x, old); // if img changed then delete old file
            await this.dataSource.getRepository(CShopitem).save(x);
            return {statusCode: 200, data: x};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopitemsService.update", err);
            return {statusCode: 500, error};
        }
    }
