...
export class CArticlesService {
    ...
    public async chunk(dto: IGetList, user_id: number): Promise<IResponse<IArticle[]>> {
        try {
            ...
            if (user_id) {
                query = query.loadRelationCountAndMap("articles.readings_count", "articles.readings", "readings", qb => qb.where(`readings.user_id='${user_id}'`)) // отметки о прочтении
            }
    ...
    public async one(slug: string, user_id: number): Promise<IResponse<IArticle>> {
        try {
            const article = await this.dataSource.getRepository(CArticle)
                .createQueryBuilder("article")
                .leftJoinAndSelect("article.translations", "translations")
                .loadRelationCountAndMap("article.readings_count", "article.readings", "readings", qb => qb.where(`readings.user_id='${user_id}'`)) // отметки о прочтении
    ...
    public async updateReading(dto: IReadingUpdate, user_id: number): Promise<IResponse<void>> {
        try {
            if (dto.was_read) {
                const reading = this.dataSource.getRepository(CReading).create({user_id, article_id: dto.article_id});
                await this.dataSource.getRepository(CReading).save(reading);
            } else {
                await this.dataSource.getRepository(CReading).delete({user_id, article_id: dto.article_id});
            }

            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CArticlesService.updateReading", err);
            return {statusCode: 500, error};
        }
    }
