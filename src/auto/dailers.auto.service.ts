...
export class CDailersAutoService {
    ...
    private async resetCompletion(): Promise<void> {
        try {
            ...
            const users = await this.dataSource.getRepository(CUser).find({where: {tz: tz.offset, active: true}});
            const user_ids = users.map(u => u.id);
            // reset "dailers" completion for users
            await this.dataSource.getRepository(CDailer).update({user_id: In(user_ids)}, {completed: false});
        } catch (err) {
