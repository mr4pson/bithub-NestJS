...
export class COutordersService {
    ...
    public async create(dto: IOutorderCreate, user_id: number): Promise<IResponse<void>> {
        try {
            ...
            const tariff = await this.dataSource.getRepository(CTariff).findOne({where: {id: dto.tariff_id}, relations: ["translations"]});
            if (!tariff) return {statusCode: 404, error: "tariff not found"};
            let promocode: CPromocode = null;

            if (dto.code) {
                promocode = await this.dataSource.getRepository(CPromocode).findOneBy({code: dto.code});
                if (!this.promocodesService.check(promocode)) return {statusCode: 409, error: "code is invalid"};
            }
            ...
            if (amount > user.money) return {statusCode: 410, error: "insufficient money"};
            ...
            const outorder = this.dataSource.getRepository(COutorder).create({user_email: user.email, amount, purpose, promocode: dto.code});
            await this.dataSource.getRepository(COutorder).save(outorder);

            // update user
            user.money -= amount;

            if (tariff.type === "subscription") { // subscription
                const now = new Date();
                user.paid_at = now;
                if (!user.paid_until || user.paid_until.getTime() < now.getTime()) user.paid_until = new Date();
            ...
            if (tariff.type === "onetime") { // one-time purchase
                if (tariff.code === "subacc") { // children limit increasion
            ...
            const error = await this.errorsService.log("api.mainsite/COutordersService.create", err);
