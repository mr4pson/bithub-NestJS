...
export class CArticlesService extends CImagableService {
    ...
    public async chunk(dto: IGetList): Promise<IResponse<CArticle[]>> {
        try {
            ...
            const predata = await this.dataSource.getRepository(CArticle)
            ...
            const data = await this.dataSource.getRepository(CArticle)
                .createQueryBuilder("articles")
                ...
                .orderBy({[sortBy]: sortDir})
                .getMany();
            const elementsQuantity = await this.dataSource.getRepository(CArticle)
                .createQueryBuilder("articles")
            ...
            const error = await this.errorsService.log("api.admin/CArticlesService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<CArticle>> {
        try {
            const data = await this.dataSource.getRepository(CArticle).findOne({where: {id}, relations: ["translations"]});
            return data ? {statusCode: 200, data} : {statusCode: 404, error: "article not found"};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CArticlesService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CArticlesService.delete", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {
            const xl = await this.dataSource.getRepository(CArticle).findBy({id: In(ids)});
            await this.deleteUnbindedImgOnDelete(xl, false);
            await this.dataSource.getRepository(CArticle).delete(ids);
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CArticlesService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async create(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CArticle>> {
        try {
            ...
            x.slug = await this.slugService.checkSlug(this.dataSource.getRepository(CArticle), x);
            ...
            const error = await this.errorsService.log("api.admin/CArticlesService.create", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async update(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CArticle>> {
        try {
            ...
            const x = this.dataSource.getRepository(CArticle).create(dto);
            const old = await this.dataSource.getRepository(CArticle).findOneBy({id: x.id});
            await this.buildImg(x, uploads);
            ...
            const error = await this.errorsService.log("api.admin/CArticlesService.update", err);
            return {statusCode: 500, error};
        }
    }
    ...
    private async tgNotifyNewarticle(article: CArticle): Promise<void> {
        try {
            ...
            const users = await this.dataSource.getRepository(CUser).find({where: {active: true, tg_id: Not(IsNull()), tg_active: true, tg_articles: true}, relations: ["lang"]});
            ...
            await this.errorsService.log("api.admin/CArticlesService.tgNotifyNewarticle", err);
        }
    }
}
