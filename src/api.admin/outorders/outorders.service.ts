...
export class COutordersService {
    ...
    public async chunk(dto: IGetList): Promise<IResponse<COutorder[]>> {
        try {            
            ...
            const error = await this.errorsService.log("api.admin/COutordersService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<COutorder>> {
        try {
            const data = await this.dataSource.getRepository(COutorder).findOne({where: {id}});
            return data ? {statusCode: 200, data} : {statusCode: 404, error: "outorder not found"};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/COutordersService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/COutordersService.delete", err);
            return {statusCode: 500, error};
        }        
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {            
            ...
            const error = await this.errorsService.log("api.admin/COutordersService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
