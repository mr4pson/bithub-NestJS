...
export class CTasksController {
    ...
    @Post("paid-one/:id")
    public paidOne(@Param("id") id: string, @Req() request: Request): Promise<IResponse<ITask>> {
    ...
    public updateCompletion(@Body() dto: ICompletionUpdate, @Req() request: Request): Promise<IResponse<void>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
    ...
    @UseGuards(CUserGuard)
    @Post("unviewed-chunk")
    public unviewedChunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<ITask[]>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
    ...
    public unviewedQuantity(@Param("favorites") favorites: string, @Req() request: Request): Promise<IResponse<number>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
    ...
    @UseGuards(CUserGuard)
    @Post("viewed")
    public viewed(@Body() dto: IViewed, @Req() request: Request): Promise<IResponse<number[]>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
