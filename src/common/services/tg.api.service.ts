...
export class CTgApiService implements OnModuleInit {
    ...
    public async onModuleInit(): Promise<void> {
        try {
            ...
            const apiId = (await this.dataSource.getRepository(CSetting).findOneBy({p: "tgapi-id"}))?.v;
            const apiHash = (await this.dataSource.getRepository(CSetting).findOneBy({p: "tgapi-hash"}))?.v;
            ...
            this.client = new TelegramClient(stringSession, parseInt(apiId), apiHash, {connectionRetries: 5});
