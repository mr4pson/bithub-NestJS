...
export class CLangsService extends CImagableService  {
    ...
    public async all(dto: IGetList): Promise<IResponse<CLang[]>> {
        try {
            const data = await this.dataSource.getRepository(CLang).find({order: {[dto.sortBy]: dto.sortDir}});
            return {statusCode: 200, data};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CLangsService.all", err);
    ...
    public async chunk(dto: IGetList): Promise<IResponse<CLang[]>> {
        try {
            const data = await this.dataSource.getRepository(CLang).find({order: {[dto.sortBy]: dto.sortDir}, take: dto.q, skip: dto.from});
            const elementsQuantity = await this.dataSource.getRepository(CLang).count();
            const pagesQuantity = Math.ceil(elementsQuantity / dto.q);
            return {statusCode: 200, data, elementsQuantity, pagesQuantity};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CLangsService.chunk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async one(id: number): Promise<IResponse<CLang>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CLangsService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CLangsService.delete", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {
            const xl = await this.dataSource.getRepository(CLang).findBy({id: In(ids)});
            await this.deleteUnbindedImgOnDelete(xl, false);
            await this.dataSource.getRepository(CLang).delete(ids);
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CLangsService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async create(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CLang>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CLangsService.create", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async update(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CLang>> {
        try {
            ...
            const x = this.dataSource.getRepository(CLang).create(dto);
            const old = await this.dataSource.getRepository(CLang).findOneBy({id: x.id});
            await this.buildImg(x, uploads);
            ...
            const error = await this.errorsService.log("api.admin/CLangsService.update", err);
            return {statusCode: 500, error};
        }
    }
    ...
    private async rebuildSlugable(x: CLang): Promise<void> {
        if (x.slugable) {
            await this.dataSource.getRepository(CLang).update({id: Not(x.id)}, {slugable: false});
        }
    }
