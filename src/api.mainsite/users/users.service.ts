...
export class CUsersService extends CImagableService {
    ...
    public async login(dto: IUserLogin): Promise<IResponse<IUserAuthData>> {
        try {
            ...
            if (!captchared) return {statusCode: 400, error: "captcha verification failed"};
            const user = await this.authorize(dto);
            if (!user) return {statusCode: 401, error: "Unauthorized"};
            const payload = {id: user.id};
            const data: IUserAuthData = {id: user.id, token: this.jwtService.sign(payload)};
    ...
    public async enterByToken(dto: IUserEnterByToken, tz: number): Promise<IResponse<IUserAuthData>> {
        try {
            const userData = await this.authService[`parse${dto.type}Token`](dto.token);
            if (!userData.email) return {statusCode: 400, error: "no email"};
            let user = await this.dataSource.getRepository(CUser).findOneBy({email: userData.email});
            ...
            if (!user) {
                user = this.dataSource.getRepository(CUser).create({
                    ...
                    password: this.authService.buildHash(this.appService.randomString(6, "full")),
                    wallet: "",
                    tz,
                })
            ...
            // user exists
            if (!user.active) return {statusCode: 402, error: "user exists but inactive"};
            const payload = {id: user.id};
            const data: IUserAuthData = {id: user.id, token: this.jwtService.sign(payload)};
            return {statusCode: 200, data};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CUsersService.enterByToken", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async one(id: number, visitor_id: number): Promise<IResponse<IUser>> {
        try {
            ...
            if (!(user.id === visitor_id || user.parent_id === visitor_id)) return {statusCode: 401, error: "no permission"}; // можно загружать только себя и потомков
            return {statusCode: 200, data: this.buildUser(user)};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CUsersService.me", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async canBeParent(uuid: string): Promise<IResponse<void>> {
        try {
            ...
            if (!user) return {statusCode: 404, error: "user not found"};
            if (user.children_limit <= user.children_q) return {statusCode: 409, error: "children limit exhausted"};
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CUsersService.canBeParent", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async isExists(uuid: string): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.mainsite/CUsersService.isExists", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async verify(dto: IUserVerify): Promise<IResponse<void>> {
        try {
            ...
            const verification = this.dataSource.getRepository(CVerification).create({login, code});
    ...
    public async register(dto: IUserRegister, tz: number): Promise<IResponse<IUserAuthData>> {
        try {
            // check captcha
            const captchared = await this.captchaService.verify(dto.captchaToken);
            if (!captchared) return {statusCode: 400, error: "captcha verification failed"};
            ...
            // check email
            let user = await this.dataSource.getRepository(CUser).findOneBy({email: dto.email});
            ...
            const verification = await this.dataSource.getRepository(CVerification).findOne({where: {login: dto.email, code: dto.code, created_at: MoreThanOrEqual(expiration)}});
            ...
            if (dto.parent_uuid) {
                ...
                if (parent.children_q >= parent.children_limit) return {statusCode: 411, error: "children limit exhausted"};
                parent_id = parent.id;
            }
    ...
    public async recover(dto: IUserRecover): Promise<IResponse<void>> {
        try {
            const user = await this.dataSource.getRepository(CUser).findOne({where: {email: dto.email, active: true}});
    ...
    public async update(fd: IJsonFormData, uploads: Express.Multer.File[], user_id: number): Promise<IResponse<IUser>> {
        try {
            ...
            const old = await this.dataSource.getRepository(CUser).findOneBy({id: user.id});
    ...
    public async updatePassword(dto: IUserUpdatePassword, user_id: number): Promise<IResponse<void>> {
        try {
            // здесь не будем проверять юзера на существование и активность, потому что такие случае не пропустит CUsersGuard
            const user = await this.dataSource.getRepository(CUser)
                .createQueryBuilder("user")
                .addSelect("user.password")
                .where(`user.id='${user_id}'`)
                .getOne();

            if (!(await this.authService.compareHash(dto.oldpassword, user.password))) {
            ...
            const error = await this.errorsService.log("api.mainsite/CUsersService.updatePassword", err);
            return {statusCode: 500, error};
        }
    }

    public async childrenChunk(dto: IGetList, user_id: number): Promise<IResponse<IUser[]>> {
        try {
            ...
            const users = await this.dataSource.getRepository(CUser)
                ...
                .skip(dto.from)
                .getMany();
            const elementsQuantity = await this.dataSource.getRepository(CUser)
                .createQueryBuilder("users")
                .where(filter)
                .getCount();
            ...
            const error = await this.errorsService.log("api.mainsite/CUsersService.childrenChunk", err);
            return {statusCode: 500, error};
        }
    }

    public async refereesChunk(dto: IGetList, user_id: number): Promise<IResponse<IUser[]>> {
        try {
            const sortBy = `users.${dto.sortBy}`;
            const sortDir = dto.sortDir === 1 ? "ASC" : "DESC";
            ...
            const error = await this.errorsService.log("api.mainsite/CUsersService.refereesChunk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async deactivate(user_id: number): Promise<IResponse<void>> {
        try {
            await this.dataSource.getRepository(CUser).update({id: user_id}, {active: false});
            return {statusCode: 200};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CUsersService.deactivate", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async getTgInvite(user_id: number): Promise<IResponse<string>> {
        try {
            ...
            if (user.paid_until && user.paid_until.getTime() > new Date().getTime()) {
                ...
                if (!groupName) return {statusCode: 500, error: "no Telegram group setting"};
                const invite = await this.tgapiService.getGroupInviteLink(groupName);
                if (!invite) return {statusCode: 500, error: "Telegram API request failed"};
            ...
            const error = await this.errorsService.log("api.mainsite/CUsersService.getTgInvite", err);
            return {statusCode: 500, error};
        }
    }
    ...
    private buildChildrenFilter(dtoFilter: IKeyValue<any>, user_id: number): string {
    ...
    private buildRefereesFilter(dtoFilter: IKeyValue<any>, user_id: number): string {
    ...
    private async authorize(dto: IUserLogin): Promise<CUser> {
        ...
        if (user?.active && await this.authService.compareHash(dto.password, user.password)) {
            return user;
        }
    ...
    private getUser(identifier: number | string, field: "id" | "uuid"): Promise<CUser> {
        return this.dataSource.getRepository(CUser)
            .createQueryBuilder("user")
    ...
    private buildSafeCreate(dto: IUserRegister, tz: number, parent_id: number, referrer_id: number): CUser {
        return this.dataSource.getRepository(CUser).create({
            lang_id: dto.lang_id,
            name: dto.name,
            email: dto.email,
    ...
    private buildUser(user: CUser): IUser {
        return {
            ...
            overdue: user.paid_until && user.paid_until.getTime() < new Date().getTime(),
    ...
    private async messageOnRegister(user: CUser): Promise<void> {
        try {
            ...
            await this.errorsService.log("api.mainsite/CUsersService.messageOnRegister", err)
        }
    }
    ...
    public async canSeePaidContent(user_id: number): Promise<boolean> {
        ...
        const now = new Date();
        const user = await this.dataSource.getRepository(CUser).findOne({where: {id: user_id, active: true}, relations: ["parent"]});
        if (!user) return false;
        if (!user.parent_id) return user.paid_until && user.paid_until.getTime() > now.getTime();
        return user.parent.active && user.parent.paid_until && user.parent.paid_until.getTime() > now.getTime();
    }
}