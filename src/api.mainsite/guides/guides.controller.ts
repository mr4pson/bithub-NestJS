import { Controller, Post, Body, UseGuards, Param, Req, ParseIntPipe } from "@nestjs/common";
...
export class CGuidesController {
    ...
    @Post("chunk")
    public chunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<IGuide[]>> {
    ...
    @Post("stat-chunk")
    public statChunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<IGuide[]>> {
        const token = request.headers["token"] as string;
        const visitor_id = token ? this.jwtService.decode(token)["id"] as number : null;
    ...
    public statCmpletions(@Param("id", ParseIntPipe) id: number, @Req() request: Request): Promise<IResponse<ICompletion[]>> {
        const token = request.headers["token"] as string;
        const visitor_id = token ? this.jwtService.decode(token)["id"] as number : null;
    ...
    public favoritesChunk(@Body() dto: IGetList, @Req() request: Request): Promise<IResponse<IGuide[]>> {
        const token = request.headers["token"] as string;
    ...
    @UseGuards(CUserGuard)
    @Post("one/:id")
    public one(@Param("id") id: string, @Req() request: Request): Promise<IResponse<IGuide>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
    ...
    public updateFavorition(@Body() dto: IFavoritionUpdate, @Req() request: Request): Promise<IResponse<void>> {
        const token = request.headers["token"] as string;
        const visitor_id = this.jwtService.decode(token)["id"] as number;
