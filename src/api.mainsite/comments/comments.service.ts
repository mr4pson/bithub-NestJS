...
export class CCommentsService {
    ...
    public async chunk(dto: IGetList, tz: number): Promise<IResponse<IComment[]>> {
        try {
            const filter = {active: true, guide_id: dto.filter.guide_id};
            const comments = await this.dataSource.getRepository(CComment).find({where: filter, take: dto.q, skip: dto.from, order: {[dto.sortBy]: dto.sortDir}, relations: ["user"]});
            const data = comments.map(c => this.buildComment(c, tz));
            const elementsQuantity = await this.dataSource.getRepository(CComment).count({where: filter});
            const pagesQuantity = Math.ceil(elementsQuantity / dto.q);
            return {statusCode: 200, data, elementsQuantity, pagesQuantity};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CCommentsService.chunk", err);
            return {statusCode: 500, error};
        }
    }

    public async create(user_id: number, dto: ICommentCreate): Promise<IResponse<void>> {
        try {
            ...
            const created = await this.dataSource.getRepository(CComment).save(comment);
            this.notifyOnCreate(created);
            return {statusCode: 201};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CCommentsService.create", err);
            return {statusCode: 500, error};
        }
    }
    ...
    private buildComment(comment: CComment, tz: number): IComment {
        return {
            ...
            userLetter: comment.is_admin ? "A" : comment.user.name?.substring(0, 1).toUpperCase(),
            content: comment.content,
            created_at: this.appService.humanDate(this.appService.utcToLocal(comment.created_at, tz), true),
        }
    }

    private buildCreation(user_id: number, dto: ICommentCreate): Partial<CComment> {
        return {
            is_admin: false,
            user_id,
    ...
    private async notifyOnCreate(comment: CComment): Promise<void> {
        try {
            ...
            await this.errorsService.log("api.mainsite/CCommentsService.notifyOnCreate", err);
        }
    }
}
