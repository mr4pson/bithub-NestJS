...
export class CUsersAutoService {
    ...
    private async resetFreetasksCounters(): Promise<void> {
        try {
            const sTimeout = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-reset-freetask-timeout"}}))?.v;
            if (!sTimeout) return;
            ...
            await this.dataSource
                ...
                .createQueryBuilder()
                .update({freetasks: 0})
                .where(`freetasks > 0 AND (freetask_viewed_at IS NULL OR freetask_viewed_at < '${this.appService.mysqlDate(datelimit)}')`)
                .execute();
        } catch (err) {
            await this.errorsService.log("CUsersAutoService.resetFreetasksCounters", err);
        }
    }
    ...
    private async noSubscriptionReminder(): Promise<void> {
        try {
            const sEnabled = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nosubs-reminder"}}))?.v;
            const sTimeout = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nosubs-reminder-timeout"}}))?.v; // через сколько дней после регистрации человек получит письмо
            ...
            const filter = `users.active='1' AND users.paid_until IS NULL AND users.created_at >= '${this.appService.mysqlDate(from, "datetime")}' AND users.created_at < '${this.appService.mysqlDate(to, "datetime")}'`;
            ...
            await this.errorsService.log("CUsersAutoService.noSubscriptionReminder", err);
        }
    }
    ...
    private async noSubscriptionPromo(): Promise<void> {
        try {
            const sEnabled = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nosubs-promo"}}))?.v;
            const sDiscount = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nosubs-promo-discount"}}))?.v;
            const discount = sDiscount ? parseInt(sDiscount) : 20; // если нет параметра, то 20%
            const sTimeout = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nosubs-promo-timeout"}}))?.v; // через сколько дней после регистрации человек получит письмо
            ...
            await this.errorsService.log("CUsersAutoService.noSubscriptionPromo", err);
        }
    }
    ...
    private async nopay(): Promise<void> {
        try {
            const sEnabled = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nopay-reminder"}}))?.v;
            const sTimeout = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "site-nopay-reminder-timeout"}}))?.v; // через сколько минут после попытки оплаты человек получит письмо
            ...
            const filter = `inorders.completed='0' AND inorders.created_at >= '${this.appService.mysqlDate(from, "datetime")}' AND inorders.created_at < '${this.appService.mysqlDate(to, "datetime")}'`;
            ...
            for (let inorder of inorders) {
                const user = await this.dataSource.getRepository(CUser).findOne({where: {email: inorder.user_email, active: true}});
                if (!user) continue;
    ...
    private async deadlines(): Promise<void> {
        try {
            ...
            const tasks = await this.dataSource
                ...
                .where(`guide.active='1' AND tasks.actual_until='${this.appService.mysqlDate(date, "datetime")}'`)
                .getMany();

            for (let task of tasks) {
                // юзеры, у которых этот гайд в избранном, есть подписка на уведомления и оплачен тариф
                const filter = `users.active = '1' AND users.tg_id IS NOT NULL AND users.tg_active = '1' AND users.tg_deadlines = '1' AND users.paid_until IS NOT NULL AND users.paid_until > '${this.appService.mysqlDate(now, "datetime")}' AND favoritions.guide_id = '${task.guide_id}'`;
                const users = await this.dataSource
                    .getRepository(CUser)
                    .createQueryBuilder("users")
                ...
                for (let user of users) {
                    ...
                    const completion = await this.dataSource.getRepository(CCompletion).findOne({where: {user_id: user.id, task_id: task.id}});
                    // выполнен
    ...
    private dmRemindHour: number = 8;
    private dmRemindMinute: number = 0;
    ...
    private async dmRemind(): Promise<void> {
        try {
            ...
            for (let timezone of timezones) {
                ...
                if (targetTime.getHours() !== this.dmRemindHour || targetTime.getMinutes() !== this.dmRemindMinute) continue;
                // какая дата сейчас у юзеров с таким офсетом?
                const userDate = this.appService.mysqlDate(this.appService.utcToLocal(now, offset));
                ...
                const datemarks = await this.dataSource
                    ...
                    .where(`datemarks.date='${userDate}' AND datemarks.type='reminder' AND user.active='1' AND user.tg_active='1' AND user.tz='${offset}' AND user.paid_until IS NOT NULL AND user.paid_until > '${this.appService.mysqlDate(now, "datetime")}'`)
                    .getMany();

                for (let d of datemarks) {
