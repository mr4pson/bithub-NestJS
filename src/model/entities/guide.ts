...
export class CGuide extends CEntity implements IImagable {
    ...
    @Index()
    @Column({type: "enum", enum: ["none", "drop", "possible_drop", "early_access", "points"], nullable: false, default: "none"})
    public earnings: TGuideEarnings;

    @Index()
    @Column({type: "enum", enum: ["current", "ending", "expired"], nullable: false, default: "current"})
    ...
    @OneToMany(type => CGuideTranslation, translation => translation.guide, {cascade: true})
    ...
    @ManyToOne(type => CCat, {onDelete: "SET NULL", onUpdate: "CASCADE", cascade: false})
    @JoinColumn({name: "cat_id"})
    public cat: CCat;

    @OneToMany(type => CFavorition, favorition => favorition.guide, {cascade: false})
    ...
    public fakeInit(cat_id: number, counter: number): CGuide {
        ...
        // time and time
        this.time = this.tasks.filter(t => t.type === "main").reduce((acc, t) => acc + t.time, 0);
        this.price = this.tasks.filter(t => t.type === "main").reduce((acc, t) => acc + t.price, 0);
        return this;
    }
}
