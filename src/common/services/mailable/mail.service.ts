...
export class CMailService extends CMailableService {
    ...
    public async adminProposal(email: string, proposal: CProposal): Promise<void> {
        try {
    ...
    public async adminShoporder(email: string, shoporder: CShoporder): Promise<void> {
        try {
    ...
    public async userNosubscriptionReminder(user: CUser): Promise<void> {
        try {
            const mtd = await this.getMailtemplateData("user-nosubs-reminder", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl;   // will use in eval
            const subject = mtd.subject;
            const content = eval("`"+mtd.content+"`");
            await this.sendMessage(user.email, subject, content);
        } catch (err) {
            await this.errorsService.log("CMailService.userNosubscriptionReminder", err);
        }
    }

    public async userNosubscriptionPromo(user: CUser, promocode: CPromocode): Promise<void> {
        try {
            const mtd = await this.getMailtemplateData("user-nosubs-promo", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl;   // will use in eval
            const subject = mtd.subject;
            const content = eval("`"+mtd.content+"`");
            await this.sendMessage(user.email, subject, content);
    ...
    public async userNopayReminder(user: CUser): Promise<void> {
        try {
            const mtd = await this.getMailtemplateData("user-nopay-reminder", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl;   // will use in eval
            const subject = mtd.subject;
            const content = eval("`"+mtd.content+"`");
            await this.sendMessage(user.email, subject, content);
    ...
    public async userSubscription(user: CUser): Promise<void> {
        try {
            const mtd = await this.getMailtemplateData("user-subscription", user.lang_id);
            const subject = mtd.subject;
            const content = eval("`"+mtd.content+"`");
            await this.sendMessage(user.email, subject, content);
        } catch (err) {
    ...
    public async userSubscriptionEnds(user: CUser): Promise<void> {
        try {
            const mtd = await this.getMailtemplateData("user-subscription-ends", user.lang_id);
            const mainsiteUrl = cfg.mainsiteUrl;   // will use in eval
            const date = this.appService.humanDate(this.appService.utcToLocal(user.paid_until, user.tz), true); // will use in eval
            const subject = mtd.subject;
            const content = eval("`"+mtd.content+"`");
            await this.sendMessage(user.email, subject, content);
        } catch (err) {
    ...
    public async sendMessage(to: string, subject: string, html: string): Promise<void> {
        const host = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-host"}}))?.v;
        const port = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-port"}}))?.v;
        const login = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-login"}}))?.v;
        const from = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-from"}}))?.v;
        const pw = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-pw"}}))?.v;
        const secure = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-secure"}}))?.v;
        const hostname = (await this.dataSource.getRepository(CSetting).findOne({where: {p: "smtp-hostname"}}))?.v;
        ...
        console.log(new Date(), "CMailService.sendMessage response:", data.response);
    }
}
