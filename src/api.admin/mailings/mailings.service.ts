...
export class CMailingsService {
  ...
  public async chunk(dto: IGetList): Promise<IResponse<CMailing[]>> {
    try {
      const data = await this.dataSource.getRepository(CMailing).find({
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.chunk',
        err,
      );
  ...
  public async one(id: number): Promise<IResponse<CMailing>> {
    try {
      ...
      return data
        ? { statusCode: 200, data }
        : { statusCode: 404, error: 'mailing not found' };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.one',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async oneShort(id: number): Promise<IResponse<CMailing>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.oneShort',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async delete(id: number): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.delete',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.deleteBulk',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async create(fd: IJsonFormData): Promise<IResponse<CMailing>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.create',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async update(fd: IJsonFormData): Promise<IResponse<CMailing>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.update',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async run(id: number): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CMailingsService.run',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  private async send(mailing: CMailing): Promise<void> {
    try {
      ...
      if (!recipients?.length) {
        await this.dataSource
          .getRepository(CMailing)
          .update({ id: mailing.id }, { status: 'idle', running_status: null });
        return;
      }
      ...
      for (let i = 0; i < recipients.length; i++) {
        await this.dataSource
          ...
          .update(
            { id: mailing.id },
            { running_status: `(${i + 1}/${recipients.length})` },
          );
      ...
      await this.dataSource
        .getRepository(CMailing)
        .update({ id: mailing.id }, { status: 'error', running_status: null });
    }
  }
}
