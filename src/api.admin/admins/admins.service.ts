...
export class CAdminsService extends CImagableService {
    ...
    public async chunk(dto: IGetList): Promise<IResponse<CAdmin[]>> {
        try {
            const data = await this.dataSource.getRepository(CAdmin).find({where: {hidden: false}, order: {[dto.sortBy]: dto.sortDir}, take: dto.q, skip: dto.from, relations: ["group"]});
            const elementsQuantity = await this.dataSource.getRepository(CAdmin).count({where: {hidden: false}});
            const pagesQuantity = Math.ceil(elementsQuantity / dto.q);
            return {statusCode: 200, data, elementsQuantity, pagesQuantity};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CAdminsService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<CAdmin>> {
        try {
            const data = await this.dataSource.getRepository(CAdmin).findOneBy({id});
            return data ? {statusCode: 200, data} : {statusCode: 404, error: "admin not found"};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CAdminsService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CAdminsService.delete", err);
            return {statusCode: 500, error};
        }        
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {   
            ...
            const error = await this.errorsService.log("api.admin/CAdminsService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async create(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CAdmin>> {        
        try {    
            ...
            const error = await this.errorsService.log("api.admin/CAdminsService.create", err);
            return {statusCode: 500, error};
        }        
    }
    ...
    public async update(fd: IJsonFormData, uploads: Express.Multer.File[]): Promise<IResponse<CAdmin>> {
        try {
            ...
            const x = this.dataSource.getRepository(CAdmin).create(dto);
            const old = await this.dataSource.getRepository(CAdmin).findOneBy({id: x.id});
            await this.buildImg(x, uploads);
            ...
            const error = await this.errorsService.log("api.admin/CAdminsService.update", err);
            return {statusCode: 500, error};
        } 
    }   
    ...
    public async login(dto: IAdminLogin): Promise<IResponse<IAdminAuthData>> {
        try {            
            ...
            const error = await this.errorsService.log("api.admin/CAuthService.login", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async verify(dto: IAdminVerify): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CAuthService.verify", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async recover(dto: IAdminRecovery): Promise<IResponse<void>> {
        try {
            const admin = await this.dataSource.getRepository(CAdmin).findOne({where: {email: dto.email}});
            ...
            const error = await this.errorsService.log("api.admin/CAuthService.recover", err);
            return {statusCode: 500, error};
        }
    }  
    ...
    protected async authorize(email: string, password: string): Promise<CAdmin> {        
        ...
        if (admin?.active && await this.authService.compareHash(password, admin.password)) {            
            return admin;
        } 
