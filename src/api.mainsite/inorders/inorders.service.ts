...
export class CInordersService {
  ...
  public async complete(dto: IWpEvent, signature: string): Promise<string> {
    try {
      ...
      if (dto.event_type === 'transaction::completed') {
        ...
        const amount =
          parseFloat(dto.transaction.value) * parseFloat(order.exchange_rate); // фактически полученная сумма: сумма транзакции на курс валют из ордера
        const inorder = await this.dataSource
          .getRepository(CInorder)
          .findOne({ where: { outer_id: order.id } });
        ...
        if (referrer && referrer.active && referrer.referral_percent) {
          const otkat = parseFloat(
            ((amount / 100) * referrer.referral_percent).toFixed(),
          );
        ...
        order.status !== 'COMPLETE' &&
          (await this.whitepayCompleteOrder(order.id));
      }
  ...
  private async whitepayGetOrder(order_id: string): Promise<IWpOrder> {
    let url = (
      await this.dataSource
        .getRepository(CSetting)
        .findOneBy({ p: 'whitepay-url-get-order' })
    )?.v;
    const token = (
      await this.dataSource
        .getRepository(CSetting)
        .findOneBy({ p: 'whitepay-token' })
    )?.v;
  ...
  private async whitepayCompleteOrder(order_id: string): Promise<IWpOrder> {
    let url = (
      await this.dataSource
        .getRepository(CSetting)
        .findOneBy({ p: 'whitepay-url-complete-order' })
    )?.v;
    const token = (
      await this.dataSource
        .getRepository(CSetting)
  ...
  private test(): void {
    ...
    const computedSignature = HmacSHA256(
      payloadJson,
      '43f5fd17dc31ac654af671e1d753e4e9ac80af5bc0ac6a51b8988210aac25ed9',
    ).toString();
