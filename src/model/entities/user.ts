...
export class CUser extends CEntity implements IChildable, IImagable {
    ...
    @ManyToOne(type => CLang, {onDelete: "RESTRICT", onUpdate: "CASCADE", cascade: false})
    ...
    @ManyToOne(type => CUser, {onDelete: "SET NULL", onUpdate: "CASCADE", cascade: false})
    ...
    @OneToMany(type => CCompletion, completion => completion.user, {cascade: false})
    public completions: CCompletion[];

    @OneToMany(type => CFavorition, favorition => favorition.user, {cascade: false})
    ...
    public fakeInit(counter: number, parent_id: number, referrer_id: number, hasher: (pw: string) => string): CUser {
        this.lang_id = 1;
