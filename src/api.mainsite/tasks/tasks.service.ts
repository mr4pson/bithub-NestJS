...
export class CTasksService {
  ...
  public async unviewedChunk(
    ...
    try {
      ...
      const tasks = await this.dataSource.getRepository(CTask).find({
        where: { id: In(task_ids) },
        relations: ['translations', 'guide', 'guide.translations'],
      });
      const query3 = `SELECT COUNT(*) AS count FROM (${query1}) AS x`;
      const elementsQuantity = (await this.dataSource.query(query3))[0][
        'count'
      ];
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.unviewedChunk',
        err,
      );
  ...
  public async unviewedQuantity(
    ...
    try {
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.unviewedQuantity',
        err,
      );
      return { statusCode: 500, error };
    }
    ...
    try {
      ...
      const guides = await this.dataSource
        ...
        .leftJoinAndSelect(
          'guides.tasks',
          'tasks',
          `tasks.created_at > '${this.appService.mysqlDate(user.created_at)}'`,
        )
        .leftJoin('guides.favoritions', 'favoritions')
        .where(filter)
        .getMany();
      const newViewings: CViewing[] = []; // создаваемые просмотры

      for (const guide of guides) {
        const existedViewings = await this.dataSource
          .getRepository(CViewing)
          .find({
            where: { user_id, task_id: In(guide.tasks.map((t) => t.id)) },
          });
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.unviewedViewed',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async paidOne(id: number, user_id: number): Promise<IResponse<ITask>> {
    try {
      ...
      let user = await this.dataSource.getRepository(CUser).findOne({
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.paidOne',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async one(id: number): Promise<IResponse<ITask>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.one',
        err,
      );
      return { statusCode: 500, error };
    }
    ...
    try {
      if (dto.completed) {
        const completions = await this.dataSource
          ...
          .find({ where: { user_id, task_id: dto.task_id } });

        if (!completions.length) {
          const completion = this.dataSource
            .getRepository(CCompletion)
            .create({ user_id, task_id: dto.task_id });
          await this.dataSource.getRepository(CCompletion).save(completion);
        }
      } else {
        await this.dataSource
          .getRepository(CCompletion)
          .delete({ user_id, task_id: dto.task_id });
      }

      return { statusCode: 200 };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.mainsite/CTasksService.updateCompletion',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  private async buildUnviewedChunkFilter(
    dtoFilter: any,
    user: CUser,
  ): Promise<string> {
    let filter = `
    ...
    if (dtoFilter.created_at_less) {
      filter += ` AND t.created_at <= '${this.appService.mysqlDate(
  ...
  private async buildUnviewedQuantityFilter(
    favorites: boolean,
    user: CUser,
  ): Promise<string> {
    let filter = `
  ...
  private buildViewedFilter(dto: IViewed, user_id: number): string {
    ...
    if (dto.guide_id) {
      // в этом случае просто один гайд
