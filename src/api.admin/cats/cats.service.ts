...
export class CCatsService {
  ...
  public async chunk(dto: IGetList): Promise<IResponse<CCat[]>> {
    try {
      const data = await this.dataSource.getRepository(CCat).find({
  ...
  public async one(id: number): Promise<IResponse<CCat>> {
    try {
      ...
      return data
        ? { statusCode: 200, data }
        : { statusCode: 404, error: 'cat not found' };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.admin/CCatsService.one',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CCatsService.deleteBulk',
        err,
      );
      return { statusCode: 500, error };
    }
