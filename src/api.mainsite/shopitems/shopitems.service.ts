...
export class CShopitemsService {
    ...
    public async chunk(dto: IGetList): Promise<IResponse<IShopitem[]>> {
        try {
            ...
            const error = await this.errorsService.log("api.mainsite/CShopitemsService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<IShopitem>> {
        try {
            const shopitem = await this.dataSource.getRepository(CShopitem).findOne({where: {id, active: true}, relations: ["translations"]});
            if (!shopitem) return {statusCode: 404, error: "shopitem not found"};
            const langs = await this.dataSource.getRepository(CLang).find({where: {active: true}});
            const data = this.buildShopitemFull(shopitem, langs);
            return {statusCode: 200, data};
        } catch (err) {
            const error = await this.errorsService.log("api.mainsite/CShopitemsService.one", err);
            return {statusCode: 500, error};
        }
    }
