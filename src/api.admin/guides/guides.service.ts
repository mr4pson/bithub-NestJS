...
export class CGuidesService extends CImagableService {
  protected entity = 'CGuide';
  protected folder = 'guides';
  ...
  public async chunk(dto: IGetList): Promise<IResponse<CGuide[]>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.chunk',
        err,
      );
  ...
  public async one(id: number): Promise<IResponse<CGuide>> {
    try {
      ...
      return data
        ? { statusCode: 200, data }
        : { statusCode: 404, error: 'guide not found' };
    } catch (err) {
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.one',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async delete(id: number): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.delete',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.deleteBulk',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async create(
    ...
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.create',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  public async update(
    ...
    try {
      ...
      const error = await this.errorsService.log(
        'api.admin/CGuidesService.update',
        err,
      );
      return { statusCode: 500, error };
    }
  ...
  private deleteUnbindedLinks(): Promise<DeleteResult> {
    return this.dataSource
      .getRepository(CGuideLink)
      .delete({ guide_id: IsNull() });
  }
  ...
  private async tgNotifyNewtask(
    ...
    try {
      ...
      const users = await this.dataSource
        ...
        .where(
          `users.active='1' AND users.tg_id IS NOT NULL AND users.tg_active='1' AND users.tg_tasks='1' AND users.paid_until IS NOT NULL AND users.paid_until > '${this.appService.mysqlDate(
            now,
            'datetime',
          )}' AND favoritions.guide_id=${guide.id}`,
        )
        .getMany();
      ...
      await this.errorsService.log(
        'api.admin/CGuidesService.tgNotifyNewTask',
        err,
      );
    }
  }
  ...
  private async tgNotifyNewguide(guide: CGuide): Promise<void> {
    try {
      ...
      await this.errorsService.log(
        'api.admin/CGuidesService.tgNotifyNewGuide',
        err,
      );
    }
  }
