...
export class CShopordersService {
    ...
    public async chunk(dto: IGetList): Promise<IResponse<CShoporder[]>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CShopordersService.chunk", err);
    ...
    public async one(id: number): Promise<IResponse<CShoporder>> {
        try {
            const data = await this.dataSource.getRepository(CShoporder).findOne({where: {id}});
            return data ? {statusCode: 200, data} : {statusCode: 404, error: "shoporder not found"};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopordersService.one", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async delete(id: number): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CShopordersService.delete", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async deleteBulk(ids: number[]): Promise<IResponse<void>> {
        try {
            ...
            const error = await this.errorsService.log("api.admin/CShopordersService.deleteBulk", err);
            return {statusCode: 500, error};
        }
    }
    ...
    public async update(fd: IJsonFormData): Promise<IResponse<CShoporder>> {
        try {
            ...
            const x = this.dataSource.getRepository(CShoporder).create(dto);
            await this.dataSource.getRepository(CShoporder).save(x);
            return {statusCode: 200, data: x};
        } catch (err) {
            const error = await this.errorsService.log("api.admin/CShopordersService.update", err);
            return {statusCode: 500, error};
        }
    }
